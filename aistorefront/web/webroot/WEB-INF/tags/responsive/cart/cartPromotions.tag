...
<%@ attribute name="optimizedCart" required="false" type="com.accenture.performance.optimization.facades.data.OptimizedCartData" %>
...
<c:if test="${not empty optimizedCart.allPromotionResults }">
	<div class="cartproline">
        <spring:theme code="basket.received.promotions" />
        <ycommerce:testId code="cart_recievedPromotions_labels">
            <c:forEach items="${optimizedCart.allPromotionResults}" var="promotion">
