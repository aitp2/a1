...
import org.training.exceptions.PaymentAuthorizationException;
...
public class OrdersController extends BaseCommerceController
{
	...
	@Autowired
	private OptimizeCheckoutFacade optimizeCheckoutFacade;
	...
	/**
	 * Returns {@value org.training.v2.controller.BaseController#HEADER_TOTAL_COUNT} header with a total number of
	...
	@Secured(
	{ "ROLE_CUSTOMERGROUP", "ROLE_CLIENT", "ROLE_CUSTOMERMANAGERGROUP", "ROLE_TRUSTED_CLIENT" })
	@RequestMapping(value = "/users/{userId}/createOrders", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.CREATED)
	@ResponseBody
	public OrderWsDTO createOrders(@RequestParam(required = true) final String cartId, //NOSONAR
			@RequestParam(required = false) final String securityCode,
			@RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
			throws InvalidCartException, WebserviceValidationException //NOSONAR
	{
		if (LOG.isDebugEnabled())
		{
			LOG.info("placeOrder");
		...
		//placeorder
		final OrderData orderData = optimizeCheckoutFacade.placeOrder();
		LOG.info("====orderData.getCode()=====" + orderData.getCode());
		return getDataMapper().map(orderData, OrderWsDTO.class, fields);
	...
	@RequestMapping(value = "/users/{userId}/createOmsOrder", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.CREATED)
	@ResponseBody
	public OrderWsDTO createOrderFromRedis(@RequestParam(required = true) final String cartGuid,
			@RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
			throws InvalidCartException, WebserviceValidationException //NOSONAR
	{
		if (LOG.isDebugEnabled())
		{
			LOG.info("start create order in oms");
		}

		//placeorder
		final OrderData orderData = optimizeCheckoutFacade.placeOmsOrder(cartGuid);
		LOG.info("====orderData.getCode()=====" + orderData.getCode());
		return getDataMapper().map(orderData, OrderWsDTO.class, fields);
